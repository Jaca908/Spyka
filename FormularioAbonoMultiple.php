                ...
                <div class="col-md-12">
                    <fieldset class="group-border" id="fsEncabezado" style="background-color:white; /*display: none;*/">
                        ...
                        <div style="margin: auto; width: 100%;" class="row">
                            <div class="col-lg-1">
                                <label class="control-label" for="Name">Nº Documento:</label>
                            </div>
                            <div class="col-lg-3">
                            ...
                            <div class="col-lg-2">
                                <label class="control-label" for="Name">Tipo Documento<span style="font-size: 150%;  color: red;">*</span>:</label>
                            </div>
                            <div class="col-lg-3">
                              <input class="rb" type="radio" id="TipDoc" name="TipDoc" value="2" checked> Recibo<br>
						    <input class="rb" type="radio" id="TipDoc" name="TipDoc" value="3"> Nota Debito<br>
						    <input class="rb" type="radio" id="TipDoc" name="TipDoc" value="4"> Nota Credito<br>
                            </div>                            
                          </div>
                          <br> 
                          <div style="margin: auto; width: 100%;" class="row"> 
                        ...
                        <br>
                        <div style="margin: auto; width: 100%;" class="row">
    						<label style="margin-left:10px; color: red; float: left;" for="NumDoc">Asegúrese de que la factura se encuentra aceptada antes de hacerle un abono</label> 
                        </div>
                        <br>
                        
                        <div id="DIVInst1" style="/*display:none;*/ margin: auto; width: 100%;" class="row">
    						<label style="margin-left:10px; float: left;" for="NumDoc">&bull;Para ingresar el monto de abono, nota de credito o débito A UNA factura pulse dos veces sobre el campo 'Saldo Actual Factura' de la factura correspondiente y digite el monto.</label>
                        </div>
                        <div id="DIVInst2" style="/*display:none;*/ margin: auto; width: 100%;" class="row">
    						<label style="margin-left:10px; float: left;" for="NumDoc">&bull;Para borrar el monto de abono, nota de credito o débito de una factura pulse dos veces sobre el campo 'Abono'/'Débito'/'Crédito' de la factura correspondiente.</label>
                        </div>
                        <div id="DIVInst3" style="/*display:none;*/ margin: auto; width: 100%;" class="row">
    				<label style="margin-left:10px; float: left;" for="NumDoc">&bull;Para aplicar un abono, nota de credito A VARIAS facturas utilice en botón 'Cancelación Automática' y digite el monto.</label>
                        </div>
                        <br>
                    ...
                    <br>
							...
							<div class="row" style="margin-right:5px;">
								<div class="pull-right">
								      <div class="form-inline" >
								        <label class="control-label" id="lblAccionTD" for="NumDoc">Total a Abonar</label>
                        ...
                        <br>
                        <div style="margin: auto; width: 100%;" class="row">
                        	<button type="button" id="btnCancelacionAutomatica" onclick="CancelacionAutomatica()" style='margin-right:72px' class="btn btn-lg btn-primary pull-left">Cancelación<br>Automática</button>
                  ...
                  <div class="modal fade" id="ModalConfirmacionComprobante" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
          <h4 class="modal-title" style="font-weight: bold; color:#F0AD4E;" id="exampleModalLabel">Confirmación</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
          </div>
          <div class="modal-body" style="color:black;" id="MSJConfirmacion">
          </div>
          <div class="modal-footer">
          <button type="button" onclick="GenerarRecibo()" class="btn btn btn-warning" data-dismiss="modal">Sí</button>
          <button type="button" onclick="RecargarPagina()" class="btn btn-primary" data-dismiss="modal">No</button>
          </div>
        </div>
        </div>
      </div>
...
{   
	...
	{	
		//abrir el otro modal
		$("#MSJConfirmacion").html("¿Desea generar el comprobante de pago?");
		$("#ModalConfirmacionComprobante").modal("show");
	}
...
{
	//abrir el pdf y despues de que se abre recargar o cerrar esta pagina
				
		var IDRecibo = document.getElementById('NoRecibo').value;
		var TipoDocumento = $('input[name=TipDoc]:checked').val();
		var CedulaCliente = document.getElementById('Cedula').value;
		var NombreCliente = document.getElementById('Nombre').value;
		var Fecha = document.getElementById('Fecha').value;
		var SaldoAnterior = document.getElementById('SaldoActual').value;
		var MontoRecibo = document.getElementById('TotalesAAbonar').value;
		var SaldoActual = document.getElementById('SaldosActuales').value; 
		
		var table = $('#dtBasicExample').DataTable(); //if(GuarMod =='Guardo')
...
$('#dtBasicExample').on('dblclick', 'tbody td', function() {
  ...
  else if(table.cell(this).index().column==7)//quitar abono de la fatura
  {
	...
	var TipDoc=$('input[name=TipDoc]:checked').val();
	
	CalcularAbono(MontoAbono,TotalFinalAbono,SaldoActual,Accion,TipDoc);
  }
})
...
function CancelacionAutomatica()
{
	...
	document.getElementById("MontoAbonoCancAut").value='';
...
function AplicarAbono()
{
	...
	if((parseFloat(SaldoFactura)<parseFloat(MontoAbono))&&TipDoc!='3')
	{
...
<script>
	...
	function CalcularAbono(MontoAbono,TotalFinalAbono,TotalSaldoActual,Accion,TipDoc)
	{		
		$.ajax({
              url: 'Logica/ReciboAbono.php',
              type: 'post',
...
{		
	...
	if(CedulaCliente=='')
	{
		$("#MSJ").html("Error: Debe buscar un cliente");
        $("#ModalMSJ").modal("show");
	}
	else if(TotalFinalAbono=='0.00' || TotalFinalAbono=='')
	{
		$("#MSJ").html("Error: No existen montos de abonos asignados a facturas");
        $("#ModalMSJ").modal("show");
	}
	else
	{		
		...
		var IDRecibo=document.getElementById('NoRecibo').value;
		var FechaRecibo=document.getElementById('Fecha').value;
		var TipDoc=$('input[name=TipDoc]:checked').val();
		
		var TotalSaldoAnterior=document.getElementById('SaldoActual').value;
		var TotalSaldoActual=document.getElementById('SaldosActuales').value;
		...
		$.ajax({
	          ...
	          data: 
	          {
	            ...
	            TotalSaldoActual:TotalSaldoActual,
	            FacturasData:FacturasData,
	            btnGrabarRecibo:'btnGrabarRecibo'
	          },
	          dataType: 'json',
	          success:function(response){
...
$('input[type=radio][name=TipDoc]').change(function() {
	
var table = $('#dtBasicExample').DataTable();	
	
if (this.value == '2') 
{
    $(table.column(7).header()).text('Abono');
    $('#lblAccionTD').text('Total Abono');
    $('#lblModalAbono').text('Monto Abono');
    $('#lblModalCA').text('Monto Abono');
    $('#btnCancelacionAutomatica').show();
}
else if (this.value == '3') 
{
    $(table.column(7).header()).text('Débito');
    $('#lblAccionTD').text('Total Débito');
    $('#lblModalAbono').text('Monto Débito');
    $('#lblModalCA').text('Monto Débito');
    $('#btnCancelacionAutomatica').hide();
}
else //4
{
	$(table.column(7).header()).text('Crédito');
	$('#lblAccionTD').text('Total Crédito');
	$('#lblModalAbono').text('Monto Crédito');
	$('#lblModalCA').text('Monto Crédito');
	$('#btnCancelacionAutomatica').show();
}
